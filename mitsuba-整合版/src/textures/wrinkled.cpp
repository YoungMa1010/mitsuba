...
/*!\plugin{Wrinkled}{Procedural grid texture}
 * \order{2}
 * \parameters{
 ...
 * }
 * \renderings{
 *     \rendering{Grid texture applied to the material test object}{tex_Wrinkled}
 * }
 * This plugin implements a simple procedural grid texture with customizable
 * colors and line width.
 */
class Wrinkled : public Texture2D {
public:
	Wrinkled(const Properties &props) : Texture2D(props) {
		m_oct = props.getFloat("octave", 8.f);
		m_ro = props.getFloat("roughness", .5f);
	}

	Wrinkled(Stream *stream, InstanceManager *manager)
	...
	inline Spectrum eval(const Point2 &uv) const {
		...
		Spectrum color(Noise::turbulence(P, dpdx, dpdy, m_ro, m_oct));
		return color;
	}
	...
	Spectrum getMaximum() const {
		...
		for (int i = 0; i<SPECTRUM_SAMPLES; ++i)
			max[i] = std::max(1.f, geomsum * (3.f / 5.f));
		return max;
	}
...
class WrinkledShader : public Shader {
public:
	WrinkledShader(Renderer *renderer, Float octave, Float roughness, const Point2 &uvOffset,
	...
	void generateCode(std::ostringstream &oss,
		...
		oss << "uniform float " << evalName << "_oct;" << endl
			...
			<< "		dpdy[i] = 0.f;" << endl
			<< "	}" << endl
			<< "    Spectrum color(turbulence(P, dpdx, dpdy, " << evalName << "_ro, " << evalName << "_oct);" << endl
			<< "    return color;" << endl
			<< "}" << endl;
	}
...
Shader *Wrinkled::createShader(Renderer *renderer) const {
	return new WrinkledShader(renderer, m_oct, m_ro,
			m_uvOffset, m_uvScale);
}

MTS_IMPLEMENT_CLASS(WrinkledShader, false, Shader)
MTS_IMPLEMENT_CLASS_S(Wrinkled, false, Texture2D)
MTS_EXPORT_PLUGIN(Wrinkled, "Wrinkled texture");
MTS_NAMESPACE_END
