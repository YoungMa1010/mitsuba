...
public:
	AreaLight(const Properties &props) : Emitter(props) {
		...
		m_radiance = new ConstantSpectrumTexture(props.getSpectrum("radiance", Spectrum(1.0f)));
		m_surfaceArea = 0.0f; /// Don't know the surface area yet
	}

	AreaLight(Stream *stream, InstanceManager *manager)
		: Emitter(stream, manager) {
		m_radiance = static_cast<Texture *>(manager->getInstance(stream));
	...
	Spectrum samplePosition(PositionSamplingRecord &pRec,
		...
		return m_radiance->eval(its) * M_PI * m_surfaceArea;
	}
	...
	Spectrum evalPosition(const PositionSamplingRecord &pRec) const {
		...
		its.uv = pRec.uv;
		its.hasUVPartials = false;
		return m_radiance->eval(its) * M_PI;
	}
	...
	Spectrum sampleDirect(DirectSamplingRecord &dRec,
		...
		if (dot(dRec.d, dRec.refN) >= 0 && dot(dRec.d, dRec.n) < 0 && dRec.pdf != 0) {
			...
			its.uv = dRec.uv;
			its.hasUVPartials = false;
			return m_radiance->eval(its) / dRec.pdf;
		} else {
			dRec.pdf = 0.0f;
			return Spectrum(0.0f);
		}
	...
	void addChild(const std::string &name, ConfigurableObject *child) {
		if (child->getClass()->derivesFrom(MTS_CLASS(Texture))) {
			m_radiance = static_cast<Texture *>(child);
		} else {
...
Shader *AreaLight::createShader(Renderer *renderer) const {
	return new AreaLightShader(renderer, m_radiance->getAverage());
}
